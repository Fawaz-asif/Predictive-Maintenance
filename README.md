...
2. **Model Router** (`router.py`): Implements a two-stage routing architecture. Stage 1 detects if a failure will occur. If yes, Stage 2 identifies the specific failure mode.
3. **Web Application** (`app.py`): The Gradio frontend that handles user input, communicates with the router, and displays visually color-coded results.
